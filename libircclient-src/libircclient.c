...
void initWinsockLibrary () {
    ...
    if (err != 0) {
        logprintf(LOG_ERR, "WSAStartup failed with error: %d\n", err);
    }

    if (LOBYTE(wsaData.wVersion) != 2 || HIBYTE(wsaData.wVersion) != 2) {
        /* Tell the user that we could not find a usable */
        /* WinSock DLL.                                  */
        logprintf(LOG_ERR, "Could not find a usable version of Winsock.dll\n");
        WSACleanup();
    }
}
#endif
...
int irc_process_select_descriptors(irc_session_t * session) {
    ...
    if (likely(fdwatch_check_fd(session->sock, FDW_READ))) {
        ...
        while ((offset = libirc_findcrlf(session->incoming_buf, session->incoming_offset)) > 0) {
            ...
            if (session->incoming_offset - offset > 0)
                memmove(session->incoming_buf, session->incoming_buf + offset, (size_t)session->incoming_offset - (size_t) offset);
    ...
    if (fdwatch_check_fd(session->sock, FDW_WRITE)) {
        ...
        if (length > 0 && session->outgoing_offset - length > 0)
            memmove(session->outgoing_buf, session->outgoing_buf + length, ((size_t) session->outgoing_offset) - ((size_t)length));
...
void irc_set_verify_nick_callback(irc_session_t *session, irc_event_dcc_verify_incoming_dcc_request_t callback) {
    session->callbacks.verify_incoming_dcc_requests_req = callback;
}
