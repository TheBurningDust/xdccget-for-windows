...
#else
    // TODO: this actually makes the socket blocking (with iMode=0), with non-blocking mode nothing happens...
    u_long iMode = 0;
    int iResult;
    iResult = ioctlsocket(*sock, FIONBIO, &iMode);

    if (iResult != NO_ERROR) {
        logprintf(LOG_ERR, "ioctlsocket failed with error: %ld\n", iResult);
        return iResult;
    }
...
static struct irc_addr_t** resolve_hostname_by_dns(const char *hostname, int *numAddrs, int addressFamily)
{
    ...
    *numAddrs = 0;
    ...
    struct irc_addr_t **addresses = (struct irc_addr_t**) Calloc(((size_t)numAddresses) + 1, addressSize);
